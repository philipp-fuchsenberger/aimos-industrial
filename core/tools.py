"""
AIMOS v4.1.0 — Tool Registration
=================================
All tool registration functions extracted from main.py (CR-220).

_load_tools() registers system tools and skills on the agent.
Voice transcription helpers are also included here.
"""
...
async def transcribe_voice(agent, voice_path: str, log) -> str:
...
def load_tools(agent: AIMOSAgent):
    ...
    # CR-222: Pass agent secrets to skills so they don't need os.environ
    agent_secrets = getattr(agent, "_env_secrets", {})
    ...
    for skill_name, skill_cls in SKILL_REGISTRY.items():
        ...
        try:
            ...
            # Skill doesn't accept all params — try progressively fewer
            try:
...
Der Admin legt im Wizard fest, welche Felder der Agent aendern darf."""
    ...
    # Optional whitelist: config.allowed_agents = ["neo", "kai"] (empty = all allowed)
