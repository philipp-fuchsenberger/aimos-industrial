"""DeepSeek provider for the LLM Switchboard.

DeepSeek nutzt OpenAI-kompatibles API mit Bearer-Auth. Sehr günstig
(deepseek-chat ~$0.27/$1.10 pro 1M Tokens), gut für Code-Aufgaben
(deepseek-reasoner als Reasoning-Modell).
...
log = logging.getLogger("AIMOS.llm.deepseek")
...
async def call(
    *,
    ...
    """Call DeepSeek chat completion. Returns normalized AIMOS dict."""
    key = api_key or os.environ.get("DEEPSEEK_API_KEY")
    if not key:
        raise DeepSeekProviderError("DEEPSEEK_API_KEY not set in environment")
    ...
    for attempt in range(_MAX_RETRIES):
        try:
            ...
            if resp.status_code in _RETRY_CODES and attempt < _MAX_RETRIES - 1:
                ...
                log.warning(
                    "[deepseek] %s — retry %d/%d after %.1fs",
            ...
            if attempt < _MAX_RETRIES - 1:
                wait = _backoff_sleep_s(attempt)
                log.warning(
                    "[deepseek] connection error — retry %d/%d after %.1fs: %s",
    ...
    raise DeepSeekProviderError(
        f"DeepSeek retry budget exhausted: {last_exc}"
    ) if last_exc else DeepSeekProviderError("DeepSeek retry budget exhausted")


def _normalize(data: dict, model: str) -> dict:
    """Convert DeepSeek OpenAI-compatible response to AIMOS dict shape."""
    choices = data.get("choices", [])
    if not choices:
        return {
            "content": "", "tool_calls": [],
            "in_tokens": 0, "out_tokens": 0,
            "cost_usd": 0.0, "provider": "deepseek",
...
class DeepSeekProvider(ProviderBase):
    ...
    _INFO = ProviderInfo(
        ...
        supports_reasoning_effort=False,  # deepseek-reasoner hat eigenen Pfad
        max_concurrent=None,
    )
